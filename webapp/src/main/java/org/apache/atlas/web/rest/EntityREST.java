...
public class EntityREST {
  ...
  public AtlasEntityWithExtInfo getById(
      ...
      @QueryParam("includeClassificationType") @DefaultValue("false") boolean includeClassificationType)
      throws AtlasBaseException {
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(
                PERF_LOG, "EntityREST.getById(" + guid + ", " + minExtInfo + " )");
      }
      AtlasEntityWithExtInfo extInfo = entitiesStore.getById(guid, minExtInfo, ignoreRelationships, includeClassificationType);
      return extInfo;
    } finally {
   ...
   *
   * <p>In addition to the typeName path parameter, attribute key-value pair(s) can be provided in
  ...
  public AtlasEntityWithExtInfo getByUniqueAttributes(
    ...
    try {
      ...
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(
                PERF_LOG, "EntityREST.getByUniqueAttributes(" + typeName + "," + attributes + ")");
      }
      ...
      return entitiesStore.getByUniqueAttributes(
          entityType, attributes, minExtInfo, ignoreRelationships);
    } finally {
      AtlasPerfTracer.log(perf);
    }
  }
  ...
  /**
   * ***** Entity Partial Update - Allows a subset of attributes to be updated on an entity which is
   ...
   * <p>The REST request would look something like this
   *
   * <p>PUT /v2/entity/uniqueAttribute/type/aType?attr:aTypeAttribute=someValue
   *
   * <p>*****
   */
  @PUT
  @Path("/uniqueAttribute/type/{typeName}")
  public EntityMutationResponse partialUpdateEntityByUniqueAttrs(
      @PathParam("typeName") String typeName,
      @Context HttpServletRequest servletRequest,
  ...
  @DELETE
  @Path("/uniqueAttribute/type/{typeName}")
  public EntityMutationResponse deleteByUniqueAttribute(
      @PathParam("typeName") String typeName, @Context HttpServletRequest servletRequest)
      throws AtlasBaseException {
    Servlets.validateQueryParamLength("typeName", typeName);
  ...
  /**
   * Create new entity or update existing entity in Atlas. Existing entity is matched using its
   * unique guid if supplied or by its unique attributes eg: qualifiedName
   *
   * @param entity
   * @return EntityMutationResponse
   * @throws AtlasBaseException
  ...
  /**
   * ***** Entity Partial Update - Add/Update entity attribute identified by its GUID. Supports only
   * uprimitive attribute type and entity references. does not support updation of complex types
   * like arrays, maps Null updates are not possible *****
   */
  @PUT
  @Path("/guid/{guid}")
  public EntityMutationResponse partialUpdateEntityAttrByGuid(
      @PathParam("guid") String guid, @QueryParam("name") String attrName, Object attrValue)
      throws Exception {
    Servlets.validateQueryParamLength("guid", guid);
  ...
  @DELETE
  @Path("/guid/{guid}")
  public EntityMutationResponse deleteByGuid(@PathParam("guid") final String guid)
      throws AtlasBaseException {
    Servlets.validateQueryParamLength("guid", guid);
  ...
  public AtlasClassification getClassification(
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(
                PERF_LOG, "EntityREST.getClassification(" + guid + "," + classificationName + ")");
      }
  ...
  public AtlasClassification.AtlasClassifications getClassifications(@PathParam("guid") String guid)
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(PERF_LOG, "EntityREST.getClassifications(" + guid + ")");
      }
  ...
  public void addClassificationsByUniqueAttribute(
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(
                PERF_LOG, "EntityREST.addClassificationsByUniqueAttribute(" + typeName + ")");
      }
      ...
      if (guid == null) {
        throw new AtlasBaseException(
            AtlasErrorCode.INSTANCE_BY_UNIQUE_ATTRIBUTE_NOT_FOUND, typeName, attributes.toString());
      }
  ...
  public void addClassifications(
      @PathParam("guid") final String guid, List<AtlasClassification> classifications)
      throws AtlasBaseException {
    Servlets.validateQueryParamLength("guid", guid);
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(PERF_LOG, "EntityREST.addClassifications(" + guid + ")");
      }
  ...
  public void updateClassificationsByUniqueAttribute(
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(
                PERF_LOG, "EntityREST.updateClassificationsByUniqueAttribute(" + typeName + ")");
      }
  ...
  public void updateClassifications(
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(
                PERF_LOG, "EntityREST.updateClassifications(" + guid + ")");
      }
  ...
  public void deleteClassificationByUniqueAttribute(
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(
                PERF_LOG, "EntityREST.deleteClassificationByUniqueAttribute(" + typeName + ")");
      }
  ...
  public void deleteClassification(
      @PathParam("guid") String guid,
      @PathParam("classificationName") final String classificationName,
      @QueryParam("associatedEntityGuid") final String associatedEntityGuid)
      throws AtlasBaseException {
    Servlets.validateQueryParamLength("guid", guid);
    Servlets.validateQueryParamLength("classificationName", classificationName);
  ...
  public AtlasEntitiesWithExtInfo getEntitiesByUniqueAttributes(
      @PathParam("typeName") String typeName,
      @QueryParam("minExtInfo") @DefaultValue("false") boolean minExtInfo,
      @QueryParam("ignoreRelationships") @DefaultValue("false") boolean ignoreRelationships,
      @Context HttpServletRequest servletRequest)
    ...
    try {
      ...
      return entitiesStore.getEntitiesByUniqueAttributes(
          entityType, uniqAttributesList, minExtInfo, ignoreRelationships);
    } finally {
      AtlasPerfTracer.log(perf);
    }
  }
  ...
  @GET
  @Path("/bulk")
  public AtlasEntitiesWithExtInfo getByGuids(
  ...
  /**
   * Bulk API to create new entities or updates existing entities in Atlas. Existing entity is
   * matched using its unique guid if supplied or by its unique attributes eg: qualifiedName
   */
  @POST
  @Path("/bulk")
  ...
  @DELETE
  @Path("/bulk")
  public EntityMutationResponse deleteByGuids(@QueryParam("guid") final List<String> guids)
      throws AtlasBaseException {
    if (CollectionUtils.isNotEmpty(guids)) {
      for (String guid : guids) {
        Servlets.validateQueryParamLength("guid", guid);
  ...
  public void addClassification(ClassificationAssociateRequest request) throws AtlasBaseException {
    ...
    try {
      if (AtlasPerfTracer.isPerfTraceEnabled(PERF_LOG)) {
        perf =
            AtlasPerfTracer.getPerfTracer(
                PERF_LOG, "EntityREST.addClassification(" + request + ")");
      }
  ...
  public AtlasEntityHeaders getEntityHeaders(
      @QueryParam("tagUpdateStartTime") long tagUpdateStartTime) throws AtlasBaseException {
    AtlasPerfTracer perf = null;
    ...
    try {
      ...
      if (tagUpdateStartTime > tagUpdateEndTime) {
        throw new AtlasBaseException(
            AtlasErrorCode.BAD_REQUEST, "fromTimestamp should be less than toTimestamp");
      }
      ...
      ClassificationAssociator.Retriever associator =
          new ClassificationAssociator.Retriever(typeRegistry, auditRepository);
  ...
  public String setClassifications(AtlasEntityHeaders entityHeaders) throws AtlasBaseException {
    ...
    try {
      ...
      ClassificationAssociator.Updater associator =
          new ClassificationAssociator.Updater(typeRegistry, entitiesStore);
  ...
  private AtlasEntityType ensureEntityType(String typeName) throws AtlasBaseException {
    ...
    if (ret == null) {
      throw new AtlasBaseException(
          AtlasErrorCode.TYPE_NAME_INVALID, TypeCategory.ENTITY.name(), typeName);
    }

    return ret;
  }

  private AtlasClassificationType ensureClassificationType(String typeName)
      throws AtlasBaseException {
    AtlasClassificationType ret = typeRegistry.getClassificationTypeByName(typeName);

    if (ret == null) {
      throw new AtlasBaseException(
          AtlasErrorCode.TYPE_NAME_INVALID, TypeCategory.CLASSIFICATION.name(), typeName);
    }
  ...
  // attrs_1:qualifiedName=db1@cl1&attrs_2:qualifiedName=db2@cl1 ==> [ { qualifiedName:db1@cl1 }, {
  // qualifiedName:db2@cl1 } ]
   ...
   */
  private void validateUniqueAttribute(AtlasEntityType entityType, Map<String, Object> attributes)
      throws AtlasBaseException {
    if (MapUtils.isEmpty(attributes)) {
      throw new AtlasBaseException(
          AtlasErrorCode.ATTRIBUTE_UNIQUE_INVALID, entityType.getTypeName(), "");
    }
    ...
    for (String attributeName : attributes.keySet()) {
      ...
      if (attribute == null || !attribute.getIsUnique()) {
        throw new AtlasBaseException(
            AtlasErrorCode.ATTRIBUTE_UNIQUE_INVALID, entityType.getTypeName(), attributeName);
      }
    }
  }
}
